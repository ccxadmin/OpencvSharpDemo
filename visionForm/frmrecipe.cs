...
namespace visionForm
{
    public partial class frmrecipe : Form
    {
        ...
        public bool CreateRecipe(string recipeName)
        {
            if (dataGridView1.InvokeRequired)
            {
                dataGridView1.Invoke(new Func<string, bool>(CreateRecipe), recipeName);
            }
            else
            {
                ...
                for (int j = 0; j < dataGridView1.RowCount; j++)
                {
                    ...
                    {
                        if (MessageBox.Show("已存在相同名称配方，请确认是否替换？", "提醒",
            ...
            bool IsUse = true, bool isReplace=true)
        {
            try
            {
                ...
                if (!DirectoryNameList.Contains("Calib") || !DirectoryNameList.Contains("Config") ||
                    !DirectoryNameList.Contains("modelfile") || !FileNameList.Contains("hv_HomMat2D"))
                {
                    MessageBox.Show("配方文件不完整，加载失败！");
                    return false;
                }
                if (!Directory.Exists(path)) return false;

                bool createflag = CreateRecipe(recipeName);
        ...
        public bool SaveRecipe()
        {
            if (dataGridView1.InvokeRequired)
            {
                dataGridView1.Invoke(new  Func<bool>(SaveRecipe));
                return true;
            }
            else
            {
