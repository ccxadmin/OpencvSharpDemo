...
namespace FuncToolLib.Location
{
	...
	public class ShapeMatchTool: ToolBase
	{
			 ...
			 bool isMultipleTemplates=false)
		{
			...
			{
				//计算轮廓面积，筛选掉一些没必要的小轮廓
				if (Cv2.ContourArea(contours_img[i]) < 0 ||
							  Cv2.ContourArea(contours_img[i]) > 999999)
					continue;
				//轮廓长度不达标
				if (contours_img[i].Length < 0 || contours_img[i].Length > 99999)
					continue;
			...
			{
			    double bestScore=  shapeMatchResult.scores.Min();	//最佳得分
				...
				//将目标的重心坐标都存在数组中 
				shapeMatchResult.positions.Add(bestpos);//向数组中存放点的坐标
																	
				shapeMatchResult.rotations.Add(bestangle);//将目标的角度都存在数组中 
