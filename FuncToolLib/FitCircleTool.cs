        ...
        public override Result Run<T>(Mat gray, T obj)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dstcontours.Length; i++)
                {
                    ...
                    //拟合圆
                    double fitscore = AxisCoorditionRotation.FitCircle(dstcontours[i], ref centrep, ref radius);
                    ...
                    //圆自动校验
                    if (!AxisCoorditionRotation.CheckCircle(dstcontours[i], ref centrep, ref radius))
                        continue;

                    centrep.X += BoundingRect.X;
                    centrep.Y += BoundingRect.Y;

                    (fitCircleResult as FitCircleResult).positionData.Add(new Point2d(centrep.X, centrep.Y));

                    (fitCircleResult as FitCircleResult).radiusArray.Add(radius);

                    Cv2.Circle(dst, new CVPoint(centrep.X, centrep.Y), (int)radius, Scalar.Lime);

                    dst.drawCross(new CVPoint(centrep.X, centrep.Y), Scalar.Red, 20, 2);
                }            
                //检测区域
